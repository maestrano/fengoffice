...
{                   
    ...
    */
    ...
    protected function isValidIdentifier($id_obj) {
        $this->_log->debug(__FUNCTION__ . " start ");
        $result = !empty($id_obj) && (!empty($id_obj->_id) || (array_key_exists('_id',$id_obj) && $id_obj->_id == 0)) && array_key_exists('_deleted_flag',$id_obj) && $id_obj->_deleted_flag == 0;
        $this->_log->debug(__FUNCTION__ . " end " . $result);
        return $result;
    }
    
    protected function isDeletedIdentifier($id_obj) {
        $this->_log->debug(__FUNCTION__ . " start ");
        $result = !empty($id_obj) && (!empty($id_obj->_id) || (array_key_exists('_id',$id_obj) && $id_obj->_id == 0)) && array_key_exists('_deleted_flag',$id_obj) && $id_obj->_deleted_flag == 1;
        $this->_log->debug(__FUNCTION__ . " end " . $result);
        return $result;
    }
